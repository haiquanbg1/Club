...
const host = "http://fall2024c8g7.int3306.freeddns.org/api/v1";
...
export default function ChatPage() {
  ...
  const fetchMessages = async (offset = 0) => {
    try {
      ...
      const messages: MessageType[] = response.data.data
        .map((message: any) => ({
          ...message,
  ...
  const setScrollToBottom = () => {
    if (scrollContainerRef.current) {
      ...
      if (isFirstRender) {
        ...
        scrollContainerRef.current.scrollTop =
          scrollContainerRef.current.scrollHeight - 200; // 100px trước cuối
      }
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-[#313338] relative">
      ...
      {userProfile && friendProfile && (
        <Footer
          className="w-full mt-2 absolute bottom-9"
          socketRef={socketRef}
          setMessagesList={setMessagesList}
          userProfile={userProfile}
          friendProfile={friendProfile}
